...
vector<bbType> pfDetect(float *chns, int shrink, int modelHt, int modelWd, int stride,float cascThr, int height, int width, int nChns, PedModel pedestrianModel) {
	mat_t *mat;
	...
	//		vector<vector<double> > vecArray;
	//		//vecArray.push_back(new vector<double>());
	//		//plhs[0] = mxCreateNumericMatrix(m, 5, mxDOUBLE_CLASS, mxREAL);
	//		//double *bbs = (double*) mxGetData(plhs[0]);
	//		for (int i = 0; i < m; i++) {
	//			vector<double> tmpArray;
	//			tmpArray.push_back(cs[i] * stride);
	//			tmpArray.push_back(rs[i] * stride);
	//			tmpArray.push_back(modelWd);
	//			tmpArray.push_back(modelHt);
	//			tmpArray.push_back(hs1[i]);
	//			vecArray.push_back(tmpArray);
	//
	//			//cout << "Index " << i+1 << " " << bbs[i + 0 * m] << ", " << bbs[i + 1 * m] << ", " << bbs[i + 2 * m] << ", " << bbs[i + 3 * m] << ", " << bbs[i + 4 * m] << endl;
	//		}
	//
	//		for (int i = 0; i < m; i++) {
	//
	//			cout << "Index " << i+1 << " " << vecArray[i][0]  << ", " << vecArray[i][1] << ", " << vecArray[i][2] << ", " << vecArray[i][3] << ", " << vecArray[i][4] << endl;
	//		}
...
cv::Mat pdRgbConvert(cv::Mat image){
	...
	outputImage.data = (unsigned char*)rgbConvert( (unsigned char*) I, n, d, flag, 1.0f/255 );
