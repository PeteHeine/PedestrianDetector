...
if(~all(sz==0) && size(I,3)==1 && ~any(strcmpi(cs,{'gray','orig'})))
    I=I(:,:,[1 1 1]);
    warning('Converting image to color');
end %#ok<WNTAG>

% The image, I, is converted to LUV color space.
% Notice!: I is from now on in the LUV color space.
I=rgbConvert(I,cs);
...
%warning('I have changed the vector: "isN". The source code might be wrong'); isN = [1*ones(1,8) 9*ones(1,8) 17*ones(1,8)];
...
pyramid = struct( 'pPyramid',pPyramid, 'nTypes',nTypes, 'nScales',nScales, 'data',{data}, 'info',info, 'lambdas',lambdas, 'scales',scales, 'scaleshw',scaleshw );
